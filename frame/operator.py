...
from PyQt5.QtWidgets import QWidget, QListWidget, QHBoxLayout, QVBoxLayout, QTabWidget, QLabel, QComboBox, \
    QTableWidget, QTableWidgetItem, QHeaderView, QCheckBox, QPushButton, QAbstractItemView
from PyQt5.QtCore import Qt, pyqtSignal, QPoint
from popup.operator import EditUserInformationPopup
import settings
...
class UsersTable(QTableWidget):
        ...
        for row, user_item in enumerate(user_list):
            for col, header in enumerate(self.KEY_LABELS):
                ...
                if col == len(self.KEY_LABELS) - 1:
                    ...
                    edit_button.button_clicked.connect(self.edit_user_information)
    ...
    def item_check_changed(self, check_widget):
        ...
        try:
            r = requests.patch(
                url=settings.SERVER_ADDR + 'user/' + str(user_id) + '/?mc=' + settings.app_dawn.value('machine'),
    ...
    # 修改用户信息
    def edit_user_information(self, edit_widget):
        row, column = self._get_widget_index(edit_widget)
        user_id = self.item(row, 0).id
        print(user_id)
        # 弹窗设置当前用户信息
...
class UserManagePage(QWidget):
    def __init__(self, *args, **kwargs):
        ...
        self.users_table.network_result.connect(self.show_network_message)
