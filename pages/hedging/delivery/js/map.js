...
function reloadMap(text) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            $.each(res, function (hindx, house) {
                ...
                dataContent += "<li data-value="+ res[hindx]['id']+">" + res[hindx]['name'] + "</li>";
            });
            dataContent += '</ul></div>';
            $('.clickDetail').html(dataContent);
            cacheData = dataContent;
            adjustFrameSize();
        },
        error: function (res) {
            // console.log(res);
    ...
    option.series[0].data = storehouseData;  // 改变数值
    mapBoxEchart.setOption(option, true)
}
