$(function () {
    ...
    $('.search').on('click', 'button', function () {
        ...
        $.ajax({
            url: host + 'delivery/questions/?keyword=' + keyword,
            type: 'get',
            success: function (res) {
                // console.log(res)
                showContent(res.data)
    ...
    $('.myQuestion').on('click', 'a', function (e) {
        ...
        // 请求数据
        $.ajax({
            url: host + 'delivery/questions/?user=1',
            type: 'get',
            headers: {
                "AUTHORIZATION": token
            },
...
function showContent(discussions, message='*没有相关的记录') {
    ...
    $.each(discussions, function (index, question) {
        ...
        $.each(answers, function (answerIndex, answerItem) {
            ...
            if (answerer){answererAvatar=answerer['avatar'];answererNickName=answerer['username'] || answerer['phone']}
