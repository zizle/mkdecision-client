...
"""
import sys
from PyQt5.QtWidgets import QWidget, QHBoxLayout, QToolBox, QPushButton
from PyQt5.QtCore import Qt

import config
from widgets.base import FoldedBox, MenuScrollContainer
...
class HomePage(QWidget):
    def __init__(self, *args, **kwargs):
        ...
        # 品种列表折叠窗
        variety_box = FoldedBox()
        head1 = variety_box.addHead('第1个品种分组')
        head2 = variety_box.addHead('第2个品种分组')
        head3 = variety_box.addHead('第3个品种分组')
        body2 = variety_box.addBody(head=head2)
        buttons = [QPushButton('品种' + str(i)) for i in range(20)]
        body2.addButtons(buttons, horizontal_count=2)

        buttons = [QPushButton('品种' + str(i)) for i in range(15)]
        body1 = variety_box.addBody(head=head1)
        body1.addButtons(buttons, horizontal_count=2)

        variety_box.addStretch()
        layout.addWidget(variety_box, alignment=Qt.AlignLeft)
        self.setLayout(layout)




# class HomePage1(QScrollArea):
#     def __init__(self, *args, **kwargs):
#         super(HomePage1, self).__init__(*args, **kwargs)
#         home = QWidget()
#         layout = QGridLayout()
#         # widgets
